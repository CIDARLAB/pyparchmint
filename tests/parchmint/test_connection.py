...
def test_connectionPath_to_parchmint_v1(
    connection_target_dict, connection_path_dict, feature_dict
):
    feat1 = Feature(json_data=feature_dict)
    device = Device()
...
def test_connectionPath_parse_parchmint_v1(connection_path_dict, feature_dict):
    feat1 = Feature(json_data=feature_dict)
    device = Device()
    device.features = [feat1]
...
#     layer_dict, connection_target_dict, connection_path_dict, params_dict, feature_dict
# ):
...
#     assert c.to_parchmint_v1_x() == connection_path_dict
